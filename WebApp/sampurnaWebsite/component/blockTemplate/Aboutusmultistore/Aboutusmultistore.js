...
import Style from "./Aboutusmultistore.module.css";
...
componentDidMount(){
  ...
  render() {
	// console.log("this.state.blocks.repeatedBlocks",this.state.blocks.repeatedBlocks.length > 0);

		return (
			<div className="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center">
				<div className="row">
					<div className="col-lg-12">
						<h2 className="col-lg-12 globalMainTitle ">{this.state.blocks.blockTitle}</h2>
						<div className="col-lg-10 col-lg-offset-1">
							<p className="col-lg-12 col-md-12 col-sm-12 col-xs-12 commonPara mt20 " dangerouslySetInnerHTML={ { __html: this.state.blocks.blockSubTitle } }></p>
						</div>
					</div>
					<div className="col-lg-10 col-lg-offset-1 mt50">
						{this.state.blocks.repeatedBlocks.length > 0 ?
							this.state.blocks.repeatedBlocks.map((data,index)=>{
								var Title = parseInt(data.Title);
								// console.log("Title",typeof Title,Title)

								return(
								<div className="col-lg-4 " key={index}>
									<div className="col-lg-12">
										<img className={"multiicon globalBgColor rotateImg multiIconBorder "+Style.multiicon} src={data.Image} />
										{/* <i className={"fa fa-inbox multiicon globalBgColor multiIconBorder "+Style.multiicon} aria-hidden="true"></i> */}
									</div>
									<div className="col-lg-12">
										<h3>
										{Title ?
										<b><CountUp end={Title}/>+</b> :
										<b>{data.Title}</b>	
										}	
                                        </h3>
										<h4>{data.SubTitle}</h4>
