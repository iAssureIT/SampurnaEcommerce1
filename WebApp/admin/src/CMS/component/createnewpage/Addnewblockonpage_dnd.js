...
const getListStyle = (isDraggingOver) => ({
  ...
  //cursor : isDraggingOver ? 'hand' :"all-scroll",
  //padding: grid,
  //width: 250
});
...
export default class Addnewblockonpage extends React.Component {
	...
	onDragEnd(result) {
		console.log("destination", this.state.pageData);
		// dropped outside the list
		if (!result.destination) {
		  return;
		}
	
		const pageBlocks = reorder(
        ...
        pageData.pageBlocks = reorder(
			this.state.pageData.pageBlocks,
			result.source.index,
			result.destination.index
		  );
        this.setState({pageData},()=>{
			console.log("pagedaata",this.state.pageData)
			axios
			// .patch('/api/patch/pages/sort_blocks/'+this.state.urlParam,{"pageData":pageData.pageBlocks})
			//.patch('/api/pages/patch/sort_blocks/'+this.state.urlParam)
			.post('/api/pages/post/sort_blocks/'+this.state.urlParam)

			.then((response)=>{
				console.log("sort_response",response.data)
									   ...
									   <Droppable droppableId="dp1">
											...
											>
											{
												this.state.pageData.pageBlocks.map((result, index)=>{
													...
													return (
													<Draggable  key={Block_id} draggableId={Block_id} index={index}>
															  {(provided, snapshot) => (
																<div
																  ref={provided.innerRef}
																  {...provided.draggableProps}
																  {...provided.dragHandleProps}
																//   className="DynamicBlockcontainer"
																  style={getItemStyle(                                                            
