...
=======
  	...
  	componentWillReceiveProps(nextProps){
	 	...
	 	if (userDetails) {
			...
			.catch(error => {
				console.log("Error getNotificationCount() ==> ",error);
				if(error.message === "Request failed with status code 401"){
          		var userDetails =  localStorage.removeItem("userDetails");
          		localStorage.clear();
          		swal({  
	...
	getCompanyData(entityID) {
		...
		.then((response) => {
			console.log("getCompanyData response.data = ",response.data);
			this.setState({
  		...
  		if(userDetails){
			...
			.then(companyDetails=>{
				...
				.then(notifications => {
			  	...
			  	.catch(error => {
				 	console.log("Error Notification Unread Count => ",error);
				 	/*if(error.message === "Request failed with status code 401"){
	          		var userDetails =  localStorage.removeItem("userDetails");
	          		localStorage.clear();
	          		swal({  
			...
			.catch(error => {
				console.log("Error Company Details => ",error);
				if(error.message === "Request failed with status code 401"){
	       		var userDetails =  localStorage.removeItem("userDetails");
	       		localStorage.clear();
	       		swal({  
...
>>>>>>> Stashed changes
	...
	/*========== bellNotification() ==========*/
  	bellNotification(event) {
		...
		.catch(error => {
		  	console.log('Error bellNotification => ',error);
		  	/*if(error.message === "Request failed with status code 401"){
       		var userDetails =  localStorage.removeItem("userDetails");
       		localStorage.clear();
       		swal({  
							 				...
							 				<div className="profiledetails">
												{this.state.inAppNotifications && this.state.inAppNotifications.length > 0 
													?
								  					...
								  					:
								  						<div><div><p>You have no notifications</p></div></div>
												}
							 				</div>
						  				</div>
									</div>
