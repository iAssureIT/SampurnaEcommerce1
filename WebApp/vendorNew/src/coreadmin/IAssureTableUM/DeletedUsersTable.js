					...
					if (res.data.message == "COMPANYID_NOT_AVAILABLE") {
						...
						var tableData = tableData.map((a, i) => {
							...
							return {
								...
								lastLogin: a.lastLogin && a.lastLogin !== "-"  ? moment(a.lastLogin).format("DD-MMM-YY") : "-",
			...
			axios.post('/api/users/get/searchlist/delete', data)
				.then((res) => {
					...
					var tableData = res.data.map((a, i) => {
						console.log("Res in searchtext delete==>", a)
						var mobileNumber = a.mobNumber ? a.mobNumber : 'NA';
						return {
							_id: a._id,
				...
				.then((res) => {
					...
					var tableData = res.data.map((a, i) => {
						return {
							...
							fullName: '<div class="col-lg-12 col-md-12 col-sm-6 col-xs-6 pull-left"><b>' + a.fullName + '</b>' +
							'<p><i class="fa fa-envelope"></i> ' + a.email + '&nbsp  | &nbsp'+ a.mobNumber ? '<i class="fs16 fa fa-mobile"></i> ' + a.mobNumber + '</p>': '',
							// companyDetails: a.companyName + ' | ' + a.companyID,
							roles: a.role.map((b, j) => '  <span>' + b + ' </span>').toString(),
