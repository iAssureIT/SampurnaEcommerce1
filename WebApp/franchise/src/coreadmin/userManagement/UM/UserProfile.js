...
class EditUserProfile extends Component {
	...
	componentDidMount() {
		...
		$.validator.addMethod("alphabateAndCharacter", function (value, element, regexpr) {
			return regexpr.test(value);
		}, "Please enter aplphabates only.");
		...
		jQuery.validator.setDefaults({
		  ...
		  $("#editUser").validate({
			rules: {
			  firstname: {
				required: true,
				alphabateAndCharacter: /^[a-zA-z]+([\s][a-zA-Z]+)*$/,
				charactersLength : true
			  },
			  lastname: {
				required: true,
				regxA5: /^[a-zA-z]+([\s][a-zA-Z]+)*$/,
				charactersLength : true
			  },
			},
